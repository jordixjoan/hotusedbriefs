  ...
  if (event.type === "checkout.session.completed") {
    ...
    try {
      ...
      const extraIds = session.metadata?.extra_ids
        ? session.metadata.extra_ids.split(",").filter(Boolean)
        : [];
      ...
      if (process.env.APPS_SCRIPT_URL) {
        ...
        const googleResponse = await fetch(process.env.APPS_SCRIPT_URL, {
          method: "POST",
          headers: {
        ...
        console.log("APPS_SCRIPT_URL is not defined.");
      }
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      shipping_address_collection: {
        allowed_countries: [
          "ES", "DE", "FR", "GB", "IT", "NL", "SE", "CH",
          "BE", "PL", "AT", "IE", "DK", "FI", "PT"
        ]
      },
